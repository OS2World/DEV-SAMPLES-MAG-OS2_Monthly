...
*/
  GpiSetColor(hpsHex,HexTerrainColor(amap[hi.c][hi.r].iTerrain));
  GpiSetPattern(hpsHex,HexTerrainPattern(amap[hi.c][hi.r].iTerrain));
...
BYTE HexTerrainColor(int iTerrain) {
...
BYTE HexTerrainPattern(int iTerrain) {
...
BYTE HexTerrainVisibility(int iTerrain) {
/* This routine returns the visibility for a given terrain type. 
   This is a rudimentary initial version.  The true calculations for visibility are far 
   more complex than this.
*/
  switch (iTerrain) {
    case IDM_TER_CLEAR_GROUND:  return 0;
    case IDM_TER_ROUGH_GROUND:  return 0;
    case IDM_TER_WATER:         return 0;
    case IDM_TER_HEAVY_WOODS:   return 3;
    case IDM_TER_LIGHT_WOODS:   return 2;
    case IDM_TER_PAVEMENT:      return 0;
    case IDM_TER_BRIDGE:        return 0;
    case IDM_TER_LIGHT_BLDG:    return 1;
    case IDM_TER_MEDIUM_BLDG:   return 1;
    case IDM_TER_HEAVY_BLDG:    return 1;
    case IDM_TER_HARD_BLDG:     return 1;
  }
  return 0;
}
...
*/
  ...
  mapDefault.iTerrain=IDM_TER_CLEAR_GROUND;
...
/* Draws the combat map.
   Future enhancement: draw hex outline grid in one call to GpiPolyLine()
*/
  RECTL rcl;
  POINTL ptl={0,0};
  HPS hps;
  ...
  GpiSetPattern(hps,HexTerrainPattern(mapDefault.iTerrain));
  GpiSetColor(hps,HexTerrainColor(mapDefault.iTerrain));
  GpiMove(hps,&ptl);
  ptl.x=rcl.xRight;
  ptl.y=rcl.yTop;
  ...
  for (hi.c=0;hi.c<NUM_COLUMNS;hi.c++)
    for (hi.r=hi.c & 1;hi.r<NUM_ROWS-(hi.c & 1);hi.r+=2)
      if (amap[hi.c][hi.r].iTerrain==mapDefault.iTerrain)
        HexDraw(hpsHex,hi);
      else
        HexFillDraw(hi);
}
...
*/
  HEXINDEX hi;
  int GuessC, GuessR, c, r;

  if (ptl.x < HEX_SIDE)
    GuessC = 0;
  else
    GuessC = 2 * (ptl.x - HEX_SIDE) / (3 * HEX_SIDE) + 1;
  GuessR = ptl.y / (HEX_HEIGHT/2);

  for (c=-1;c<1;c++)
    for (r=-1;r<1;r++) {
      hi.r = GuessR + r;
      hi.c = GuessC + c;
      if (((hi.r % 2) == (hi.c % 2)) && (hi.r >= 0) && (hi.r < NUM_ROWS) && (hi.c >= 0) && (hi.c < NUM_COLUMNS))
